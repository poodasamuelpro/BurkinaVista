...
CREATE TABLE IF NOT EXISTS medias (
  ...
  ville TEXT,
  region TEXT,
  -- Contributeur (dénormalisé pour éviter les jointures inutiles)
...
-- Index GIN pour la recherche sur les tags (tableau PostgreSQL)
...
CREATE INDEX IF NOT EXISTS idx_medias_fulltext ON medias
  USING GIN(to_tsvector('french', coalesce(titre, '') || ' ' || coalesce(description, '') || ' ' || coalesce(ville, '')));
